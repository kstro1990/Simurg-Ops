...
- **Agentic MCP runs inside the binary.** With `mcpServers` on the request, the bridge writes a `{"mcpServers":…}` file and passes `--additional-mcp-config @<path>`, so the tool loop is the CLI's own — same shape as the Anthropic path, and the steps come back as `toolTrace` rather than live. Three things are load-bearing:
  - **Denial is the only real fence.** With just `--available-tools`/`--allow-tool`, a model denied a write tool **goes around it through the shell** — observed running `curl` against the Coda API directly. `--deny-tool shell/write/url` is what stops it; the CLI documents denial as winning over every allow, including `--allow-all-tools`. `allowedTools` then becomes one `--allow-tool <server>(<tool>)` each, since an approval prompt is unanswerable in non-interactive mode.
  - **The config file must live outside `-C`.** The CLI confines file access to its working directory (`tmpdir()` here) — verified: a path outside is denied, a path inside is readable. So the file goes to `data/`, not the temp dir, or the model could read the MCP credentials straight out of it. It is written 0600 and removed in a `finally`.
  - **`--disallow-temp-dir` is dropped when MCP is present.** Tool output over ~80 KB gets spilled to a temp file that the model is told to go read; blocked from it, the answer stops mid-sentence ("debo parsear el JSON…"). Listing a real Coda hits this immediately. The cost is that the model can read the system temp dir.
- The final answer is the **last turn's** `assistant.message`, keyed by `turnId`. With tools the CLI emits one per turn and the earlier ones are narration ("déjame leer el fichero…"); concatenating them all put that narration in the output. With no tools there is a single turn, so the result is unchanged.
